...
typedef struct tagFrame {
  ...
  unsigned short buffer[327680]; //图像数据，按行存储，最大支持
                                 //640x512，每个像素是一个 ushort 类型
} Frame;
...
IR_SDK_API int IRSDK_Create(int handle, T_IPADDR sIPAddr, CBF_IR cbf_stm,
                            CBF_IR cbf_cmd, CBF_IR cbf_comm, void *param = 0);
...
IR_SDK_API int IRSDK_ParamCfg(int handle, T_PARAMTYPE mParamType,
                              float f32Param);

IR_SDK_API int IRSDK_FrameConvert(Frame *pFrame, unsigned short *pGray,
                                  float f32Constrast, float f32Bright,
                                  unsigned int *pGethist,
                                  STAT_TEMPER *pFull_temper,
                                  unsigned short u16TFilterCoef);
IR_SDK_API int
IRSDK_FrameConvertDDE(Frame *pFrame, unsigned short *pGray, float f32Constrast,
                      float f32Bright, unsigned int *pGethist,
                      STAT_TEMPER *pFull_temper, unsigned short u16TFilterCoef,
                      unsigned char u8DDEcoef, unsigned char u8Gamma);

IR_SDK_API int IRSDK_Gray2Rgb(unsigned short *pGray, unsigned char *pRgb,
                              unsigned short Width, unsigned short Height,
                              int PalType, int Pal);

IR_SDK_API int IRSDK_GetPaletteJpeg(unsigned char *pPaletteJpeg,
                                    unsigned int *pJpegLen,
                                    unsigned char Method, int PalType, int Pal);
IR_SDK_API int IRSDK_GetPaletteBmp(unsigned char *pPaletteBmp,
                                   unsigned int *pBmpLen, unsigned char Method,
                                   int PalType, int Pal);
...
IR_SDK_API int IRSDK_DrawObj(Frame *pFrame, unsigned char *pRgb,
                             unsigned short width, unsigned short height,
                             STAT_OBJ *pObjStat);

IR_SDK_API int IRSDK_Rgb2Bmp(unsigned char *pBmpData, unsigned int *pLen,
                             unsigned char *pRgb, unsigned short Width,
                             unsigned short Height);
IR_SDK_API int IRSDK_Rgb2Jpeg(unsigned char *pJpegout, unsigned int *pLen,
                              int quality, unsigned char *pRgb,
                              unsigned short Width, unsigned short Height);

IR_SDK_API int IRSDK_SaveFrame2Jpeg(char *pFile, Frame *pFrame,
                                    unsigned char *pRgb, STAT_OBJ *pObj);
IR_SDK_API int IRSDK_ReadJpeg2Frame(char *pFile, Frame *pFrame,
                                    unsigned char isLoadObj, STAT_OBJ *pObj);

IR_SDK_API int IRSDK_SaveFrame2Video(char *pFile, Frame *pFrame,
                                     unsigned char Op, STAT_OBJ *pObj,
                                     unsigned char *pThreadBuf);
IR_SDK_API int IRSDK_ReadVideo2Frame(char *pFile, Frame *pFrame,
                                     unsigned int Index, unsigned char Op,
                                     T_SAVE_HEAD *pVideoHead, STAT_OBJ *pObj,
                                     unsigned char *pThreadBuf);

IR_SDK_API int IRSDK_SaveRgb2AVI(char *pFile, unsigned char *pRgb,
                                 unsigned short Width, unsigned short Height,
                                 unsigned char Op, int quality,
                                 unsigned char *pThreadBuf);

IR_SDK_API int IRSDK_SaveObj2CSV(char *pFile, unsigned char Op, STAT_OBJ *pObj,
                                 STAT_TEMPER *pGlobalTemper,
                                 unsigned char *pThreadBuf);
IR_SDK_API int IRSDK_SaveFrame2CSV(char *pFile, Frame *pFrame);
IR_SDK_API int IRSDK_SaveLine2CSV(char *pFile, Frame *pFrame, T_LINE sLine,
                                  unsigned char u8Format);
IR_SDK_API int IRSDK_SaveRect2CSV(char *pFile, Frame *pFrame, T_RECT sRect);
IR_SDK_API int IRSDK_SaveCircle2CSV(char *pFile, Frame *pFrame,
                                    T_CIRCLE sCircle);
IR_SDK_API int IRSDK_SavePolygon2CSV(char *pFile, Frame *pFrame,
                                     T_POLYGON sPolygon);

IR_SDK_API int IRSDK_MoveCtrl(int handle, T_PARAMCTRL mProtocol,
                              T_PARAMCTRL mType, unsigned int u32Param);
IR_SDK_API int IRSDK_TempCorrect(float f32Emiss, float f32Reflect, float f32Dis,
                                 STAT_TEMPER *pFull_temper);
